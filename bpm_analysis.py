...
def find_heartbeat_peaks(audio_envelope, sample_rate, min_bpm=40, max_bpm=240, start_bpm_hint=None, verbose_debug=False):
    ...
    if verbose_debug: print(f"\n--- VERBOSE DEBUG: find_heartbeat_peaks (v6.9) ---")
    ...
    while i < len(all_peaks):
        ...
        s1_s2_max_interval_sec = min(0.4, expected_rr_interval * 0.65)
        ...
        # --- Strong Peak Exception ---
        peak_to_floor_ratio = audio_envelope[current_peak_idx] / (dynamic_noise_floor.iloc[current_peak_idx] + 1e-9)
        strong_peak_override_threshold = 6.0

        if noise_confidence > 0.7 and not is_potential_s2 and peak_to_floor_ratio < strong_peak_override_threshold:
             ...
             if peak_to_floor_ratio >= strong_peak_override_threshold:
                 reason += f" | Bypassed Noise Rule (Strong Peak: {peak_to_floor_ratio:.1f}x floor)"
        ...
        if is_paired:
            ...
            s1_reason = f"S1 (Paired). {reason.lstrip(' | ')}"
...
def plot_results(audio_envelope, peaks, all_raw_peaks, analysis_data, smoothed_bpm, bpm_times, sample_rate, file_name,
    ...
    fig.update_layout(title_text=f"Heartbeat Analysis - {os.path.basename(file_name)} (v6.9)", dragmode='pan',
    ...
    fig.update_yaxes(title_text="Signal Amplitude", secondary_y=False,
                     range=[0, audio_envelope.max() * 40])  # reminder not to change this value
...
def print_and_log_chronological_data(audio_envelope, sample_rate, all_raw_peaks, analysis_data, smoothed_bpm,
    ...
    with open(output_log_path, "w", encoding="utf-8") as log_file:
        ...
        log_file.write(f"Analysis performed on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} with Engine v6.9\n\n")
...
def analyze_wav_file(wav_file_path, params, start_bpm_hint):
    ...
    print(f"\n--- Processing file: {os.path.basename(wav_file_path)} (Engine v6.9) ---")
    audio_envelope, sample_rate = preprocess_audio(wav_file_path, params['downsample_factor'], params['bandpass_freqs'],
                                                   params['save_debug_wav'])
    ...
    if len(peaks) < 2:
        ...
        plot_results(audio_envelope, peaks, all_raw_peaks, analysis_data, pd.Series(dtype=np.float64), np.array([]),
                     sample_rate, wav_file_path, params['min_bpm'], params['max_bpm'])
        return
    ...
    plot_results(audio_envelope, peaks, all_raw_peaks, analysis_data, smoothed_bpm, bpm_times, sample_rate,
                 wav_file_path, params['min_bpm'], params['max_bpm'])

    output_log_path = f"{file_name_no_ext}_Debug_Log.md"
    print_and_log_chronological_data(audio_envelope, sample_rate, all_raw_peaks, analysis_data, smoothed_bpm,
                                     output_log_path, wav_file_path)
...
class BPMApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Heartbeat BPM Analyzer v6.9")
        self.root.geometry("500x300")
        self.style = ttkb.Style(theme='minty')
        self.current_file = None
        self.params = {"downsample_factor": 100, "bandpass_freqs": (20, 150), "min_bpm": 40, "max_bpm": 240,
                       "smoothing_window_sec": 5, "save_debug_wav": True}
        self.create_widgets()
        ...
        try:
            for filename in os.listdir(os.getcwd()):
                if filename.lower().endswith(supported_extensions):
                    ...
                    self.analyze_btn.config(state=tk.NORMAL)
                    self.status_var.set(f"Auto-loaded: {os.path.basename(self.current_file)}")
                    return
        except Exception as e:
            print(f"Could not auto-find file: {e}")
        self.file_label.config(text="No file selected")
        self.analyze_btn.config(state=tk.DISABLED)
        self.status_var.set("No audio file found. Please browse.")
    ...
    def analyze(self):
        ...
        try:
            ...
            if ext.lower() != '.wav':
                if not AudioSegment: messagebox.showerror("Error",
                                                          "Pydub/FFmpeg required for non-WAV files."); self.status_var.set(
                    "Conversion failed"); return
