...
4. **Use the Application:**
    ...
    - After analysis completes, use the new quick action buttons in the GUI to:
        - **Open HTML Graph** — Open the interactive Plotly HTML report for the current file in your browser.
        - **Play WAV** — Open the processed WAV in your system's default player (or browser).
        - **Play WAV (Debug)** — Open the filtered debug WAV (if generated) in your system's default player.

5. **Labeler App (Optional):**
    - The `heartbeat_labeler.py` Dash app now exposes quick links to the processed files: Open HTML Graph, Play WAV, and Play WAV (Debug). These load the processed assets directly in your browser and require the Dash server to be running (`python heartbeat_labeler.py`).